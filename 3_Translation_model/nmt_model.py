...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder=torch.nn.LSTM(embed_size,hidden_size,bias=True,bidirectional=True)
        self.decoder=torch.nn.LSTMCell(embed_size+hidden_size,hidden_size,bias=True)
        ...
        self.combined_output_projection=torch.nn.Linear(hidden_size+hidden_size*2,hidden_size,bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed = pack_padded_sequence(X, source_lengths, batch_first=True)
        enc_hiddens,last_hidden,last_cell=self.encoder(packed)
        enc_hiddens=enc_hiddens.contiguous()
        enc_hiddens=enc_hiddens.views(-1,self.hidden_size*2)

        last_hidden=last_hidden.transpose(0,1).contiguous().view(-1,self.hidden_size*2)
        init_decoder_hidden=F.relu(self.h_projection(last_hidden))

        last_cell=last_cell.transpose(0,1).contiguous().view(-1,self.hidden_size*2)
        init_decoder_cell=F.relu(self.c_projection(last_cell))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)  # (b,1,2*h)->(b,2*h)
